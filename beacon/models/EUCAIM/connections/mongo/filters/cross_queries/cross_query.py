...
def cross_query(self, query: dict, scope: str, request_parameters: dict, dataset: str):
    ...
    def_list=[]
    if scope == 'imaging' and RequestAttributes.entry_type != imaging_confile["imaging"]["endpoint_name"] or scope == 'tumor' and RequestAttributes.entry_type != tumor_confile["tumor"]["endpoint_name"] or scope == 'disease' and RequestAttributes.entry_type != disease_confile["disease"]["endpoint_name"] or scope == 'patient' and RequestAttributes.entry_type != patient_confile["patient"]["endpoint_name"]:
