...
def get_endpoint_with_id(self, dataset: SingleDatasetResponse):
    ...
    if self.request_attributes.entry_type == patients_confile["patients"]["endpoint_name"]:
        query["patientId"] = self.request_attributes.entry_id
    # Save the include, limit and skip parameters so they are used later.
    include = self.request_attributes.qparams.query.includeResultsetResponses
    limit = self.request_attributes.qparams.query.pagination.limit
    skip = self.request_attributes.qparams.query.pagination.skip
...
def get_endpoint_cross_query(self, dataset: SingleDatasetResponse):
    ...
    .find({mapping["idq"]: self.request_attributes.entry_id, "datasetId": dataset.dataset}, {mapping["idq2"]: 1, "_id": 0})
    # Translate the ids.
    list_of_itemsfound=[]
