...
def get_phenotypic_endpoint(self, dataset: SingleDatasetResponse):
    ...
    if parameters_as_filters == True and query_parameters != {'$and': []}: # If there are request parameters that come from query string, reprocess the request parameters 
        ...
        query={}
     # Process the filters
    query = apply_filters(self, query, self.request_attributes.qparams.query.filters, query_parameters, dataset.dataset)
    ...
    skip = self.request_attributes.qparams.query.pagination.skip
...
def get_phenotypic_endpoint_with_id(self, dataset: SingleDatasetResponse):
    ...
    if self.request_attributes.entry_type == genomicVariant_confile["genomicVariant"]["endpoint_name"]:
        query = {"$and": [{"_id": self.request_attributes.entry_id}]}
...
def get_variants_of_phenotypic_endpoint(self, dataset: SingleDatasetResponse):
    ...
    if self.request_attributes.pre_entry_type == analysis_confile["analysis"]["endpoint_name"] or self.request_attributes.pre_entry_type == run_confile["run"]["endpoint_name"]:
        query = {"$and": [{"id": self.request_attributes.entry_id}]}
        query = apply_filters(self, query, self.request_attributes.qparams.query.filters, {}, dataset.dataset)
        if self.request_attributes.pre_entry_type == analysis_confile["analysis"]["endpoint_name"]:
...
def get_phenotypic_endpoint_of_variants(self, dataset: SingleDatasetResponse):
    ...
    filters=self.request_attributes.qparams.query.filters
    new_filters=[]
    if filters != []: # If there are filters, check if there are zygosity filters.
        for filter in filters:
            if filter['id']=='GENO:0000458': # If there is any filter of homozygosity, collect only those ids.
                ...
                self.request_attributes.qparams.query.filters.remove(filter)
    ...
    if self.request_attributes.entry_type != individual_confile["individual"]["endpoint_name"]:
        ...
        try:
            finalids=[]
            if self.request_attributes.entry_type == biosample_confile["biosample"]["endpoint_name"]:
                idq = 'id'
            else:
                idq = 'biosampleId'
...
def get_variants_of_dataset(self, dataset: SingleDatasetResponse):
    ...
    if dataset.dataset == self.request_attributes.entry_id:
        queryid={}
        ...
        return dataset
    # Process filters
    query = apply_filters(self, query_count, self.request_attributes.qparams.query.filters, {}, dataset.dataset)
    # Save the include, limit and skip parameters so they are used later.
    include = self.request_attributes.qparams.query.includeResultsetResponses
    limit = self.request_attributes.qparams.query.pagination.limit
    skip = self.request_attributes.qparams.query.pagination.skip
...
def get_phenotypic_endpoint_of_dataset(self, dataset: SingleDatasetResponse):
    # Process filters
    query = apply_filters(self, {}, self.request_attributes.qparams.query.filters, {}, dataset.dataset)
    # Add the dataset id requested in the query syntax.
    query = query_id(self, query, self.request_attributes.entry_id)
    # If the entry_id belongs to any dataset, build the query, otherwise, dataset is not found.
    dict_in={}
    if dataset.dataset == self.request_attributes.entry_id:
        dict_in['datasetId']=self.request_attributes.entry_id
    else:
        dataset.dataset_count=0
        dataset.exists=False
        return dataset
    # Process filters
    query = apply_filters(self, dict_in, self.request_attributes.qparams.query.filters, {}, dataset.dataset)
    # Save the include, limit and skip parameters so they are used later.
    include = self.request_attributes.qparams.query.includeResultsetResponses
    limit = self.request_attributes.qparams.query.pagination.limit
    skip = self.request_attributes.qparams.query.pagination.skip
...
def get_phenotypic_endpoint_of_cohort(self, dataset: SingleDatasetResponse):
    ...
    dataset_found = cohorts \
        .find({"id": self.request_attributes.entry_id}, {"datasetId": 1, "_id": 0})
...
def get_variants_of_cohort(self, dataset: SingleDatasetResponse):
    ...
    for doc in docs:
        if doc["datasetId"] == dataset.dataset:
            self.request_attributes.entry_id = dataset.dataset
    if dataset.dataset == self.request_attributes.entry_id:
        queryid={}
        queryid["datasetId"]=dataset.dataset
        query_count["$or"].append(queryid)
...
def get_phenotypic_cross_query(self, dataset: SingleDatasetResponse):
    ...
    .find({"id": self.request_attributes.entry_id, "datasetId": dataset.dataset}, {mapping["idq2"]: 1, "_id": 0})
    # Translate the ids.
    list_of_itemsfound=[]
