...
async def get_datasets_list(self, authorized_datasets):
    try:
        ...
        specific_datasets = self.request_attributes.qparams.query.requestParameters['datasets']
    except Exception as e:
        specific_datasets = []
    # Get the datasets from all the databases.
    list_of_datasets_confs = get_all_modules_datasets(self.request_attributes.source)
    beacon_datasets=[]
